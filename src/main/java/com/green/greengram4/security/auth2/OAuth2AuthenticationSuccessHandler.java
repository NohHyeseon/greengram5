...
public class OAuth2AuthenticationSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    private Boolean hasAuthorizedRedirectUri(String uri) {
        URI clientRedirectUri = URI.create(uri); //URI로 감싸 uri객체 생성
        log.info("clientRedriectUri.getHost(): {}", clientRedirectUri.getHost());
        log.info("clientRedriectUri.getPort(): {}", clientRedirectUri.getPort());

        return appProperties.getOauth2().getAuthorizedRedirectUris()
                .stream()
                .anyMatch(redirectUri -> {
                    URI authorizedURI = URI.create(redirectUri);
                    if (authorizedURI.getHost().equalsIgnoreCase(clientRedirectUri.getHost()) //ignorecase소문자로 바꿔서 비교함
